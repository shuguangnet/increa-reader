...
import { useConnectivity, useServiceWorkerUpdate } from '@/hooks/use-pwa'
...
function PWAUpdateBanner() {
  ...
  if ((updateStatus !== 'update-available' && updateStatus !== 'updating') || remindLater)
    return null
        ...
        <div className="bg-white dark:bg-gray-900 rounded-t-xl sm:rounded-xl w-full max-w-sm p-5 shadow-2xl animate-in slide-in-from-bottom duration-200">
          ...
          <ol className="space-y-3 text-sm text-gray-700 dark:text-gray-300">
            <li className="flex gap-3 items-start">
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">
                1
              </span>
              <span>
                点击底部工具栏的 <strong>分享按钮</strong>{' '}
                <Share className="inline size-4 text-blue-500 mx-0.5" />
              </span>
            </li>
            <li className="flex gap-3 items-start">
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">
              ...
              <span>
                在弹出的菜单中找到 <strong>「添加到主屏幕」</strong>{' '}
                <Smartphone className="inline size-4 text-green-500 mx-0.5" />
              </span>
            </li>
            <li className="flex gap-3 items-start">
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">
              ...
              <span>
                点击 <strong>「添加」</strong> 即可，Increa Reader 图标将出现在主屏幕上
              </span>
            </li>
          </ol>
...
function MobileLayout() {
  ...
  return (
    <div className="h-[calc(100%-2.75rem)] relative safe-top">
      ...
      {leftPanelVisible && (
        <SwipeableDrawer side="left" onClose={toggleLeftPanel}>
          <LeftPanel />
        </SwipeableDrawer>
      )}
      ...
      {rightPanelVisible && (
        <SwipeableDrawer side="bottom" onClose={toggleRightPanel}>
...
export function Layout() {
  ...
  const { isOver, dropHandler } = useFileDrop(
    useCallback(_files => {
