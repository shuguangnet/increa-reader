...
//
// The @ alias resolves to packages/ui/src so the desktop build compiles
// the full UI codebase. Code splitting (manualChunks) mirrors the web UI
// config to keep initial load fast and enable efficient browser caching.
export default defineConfig({
  ...
  // Output to our own dist/ so Tauri's frontendDist config finds it.
  build: {
    outDir: path.resolve(__dirname, 'dist'),
    emptyOutDir: true,
    rollupOptions: {
      // Tauri 2 injects APIs globally via withGlobalTauri: true
